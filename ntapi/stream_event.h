 ...
 * These events are not issued for fixed ports like RJ45.
   ...
   If the link goes up or down then NT_EVENT_PORT_LINK_UP and NT_EVENT_PORT_LINK_DOWN are issued.
   A special case is relevant if the link goes down and then shortly afterwards
   up again and at the same time sampling is used to detect link changes
   ...
   current sampling, and it was also up at last sampling but the sticky bit
   indicates that the link has been down, then a NT_EVENT_PORT_LINK_DOWN event
   followed immidiately by a NT_EVENT_PORT_LINK_UP event is issued.
   ...
   sampling period but this will only count as ONE link down glitch.
   Whether sampling or interrupt is used to detect link changes is adapter dependend.
 */
enum NtEventPort_e {
